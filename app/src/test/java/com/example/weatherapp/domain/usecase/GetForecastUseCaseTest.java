package com.example.weatherapp.domain.usecase;

import com.example.weatherapp.domain.model.ForecastData;
import com.example.weatherapp.domain.repository.WeatherRepository;

import org.junit.Before;
import org.junit.Test;
import org.mockito.ArgumentCaptor;
import org.mockito.Mock;
import org.mockito.MockitoAnnotations;

import static org.junit.Assert.*;
import static org.mockito.ArgumentMatchers.any;
import static org.mockito.ArgumentMatchers.anyDouble;
import static org.mockito.ArgumentMatchers.eq;
import static org.mockito.Mockito.verify;

/**
 * Unit tests for GetForecastUseCase
 */
public class GetForecastUseCaseTest {
    
    @Mock
    private WeatherRepository repository;
    
    @Mock
    private GetForecastUseCase.Callback callback;
    
    private GetForecastUseCase useCase;
    
    @Before
    public void setup() {
        MockitoAnnotations.openMocks(this);
        useCase = new GetForecastUseCase(repository);
    }
    
    @Test
    public void execute_withValidCoordinates_callsRepository() {
        // Arrange
        double latitude = 40.7128;
        double longitude = -74.0060;
        String temperatureUnit = "celsius";
        
        // Act
        useCase.execute(latitude, longitude, temperatureUnit, callback);
        
        // Assert
        verify(repository).getForecast(
                eq(latitude),
                eq(longitude),
                eq(temperatureUnit),
                any(WeatherRepository.ForecastCallback.class)
        );
    }
    
    @Test
    public void execute_withInvalidLatitude_callsOnError() {
        // Arrange
        double latitude = -100.0; // Invalid: < -90
        double longitude = 0.0;
        String temperatureUnit = "celsius";
        
        ArgumentCaptor<String> errorCaptor = ArgumentCaptor.forClass(String.class);
        
        // Act
        useCase.execute(latitude, longitude, temperatureUnit, callback);
        
        // Assert
        verify(callback).onError(errorCaptor.capture());
        assertTrue(errorCaptor.getValue().contains("Invalid"));
    }
    
    @Test
    public void execute_withInvalidLongitude_callsOnError() {
        // Arrange
        double latitude = 0.0;
        double longitude = 200.0; // Invalid: > 180
        String temperatureUnit = "celsius";
        
        ArgumentCaptor<String> errorCaptor = ArgumentCaptor.forClass(String.class);
        
        // Act
        useCase.execute(latitude, longitude, temperatureUnit, callback);
        
        // Assert
        verify(callback).onError(errorCaptor.capture());
        assertTrue(errorCaptor.getValue().contains("Invalid"));
    }
    
    @Test
    public void execute_withBoundaryCoordinates_callsRepository() {
        // Arrange - test all corners of the world
        String temperatureUnit = "fahrenheit";
        
        // Top-left corner
        useCase.execute(90.0, -180.0, temperatureUnit, callback);
        verify(repository).getForecast(
                eq(90.0),
                eq(-180.0),
                eq(temperatureUnit),
                any(WeatherRepository.ForecastCallback.class)
        );
        
        // Bottom-right corner
        useCase.execute(-90.0, 180.0, temperatureUnit, callback);
        verify(repository).getForecast(
                eq(-90.0),
                eq(180.0),
                eq(temperatureUnit),
                any(WeatherRepository.ForecastCallback.class)
        );
    }
    
    @Test
    public void execute_withDifferentTemperatureUnits_passesCorrectUnit() {
        // Arrange
        double latitude = 51.5074;  // London
        double longitude = -0.1278;
        
        // Act & Assert - celsius
        useCase.execute(latitude, longitude, "celsius", callback);
        verify(repository).getForecast(
                anyDouble(),
                anyDouble(),
                eq("celsius"),
                any(WeatherRepository.ForecastCallback.class)
        );
        
        // Act & Assert - fahrenheit
        useCase.execute(latitude, longitude, "fahrenheit", callback);
        verify(repository).getForecast(
                anyDouble(),
                anyDouble(),
                eq("fahrenheit"),
                any(WeatherRepository.ForecastCallback.class)
        );
    }
    
    @Test
    public void execute_multipleCallsWithSameCoordinates_callsRepositoryMultipleTimes() {
        // Arrange
        double latitude = 35.6762;  // Tokyo
        double longitude = 139.6503;
        String temperatureUnit = "celsius";
        
        // Act
        useCase.execute(latitude, longitude, temperatureUnit, callback);
        useCase.execute(latitude, longitude, temperatureUnit, callback);
        useCase.execute(latitude, longitude, temperatureUnit, callback);
        
        // Assert - should be called 3 times
        verify(repository, org.mockito.Mockito.times(3)).getForecast(
                eq(latitude),
                eq(longitude),
                eq(temperatureUnit),
                any(WeatherRepository.ForecastCallback.class)
        );
    }
}
